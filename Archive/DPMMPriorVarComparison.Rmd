---
title: "Prior Comparison for Well Separated Case"
...
## What happens when we used IG(5,50) prior on variance?
...
```
## What happened when we were using the IG(1,50) for previous simulations?
...
## Traceplots for well separated with IG(5,50) prior

### DEE n=100 with split/merge
...
sim_diagnostic_sum <- function(data_path, sim_dir, runs_to_plot, mod_type = "DEE"){
  ...
  diagnostic_sum[i,"MAP_K"] = as.numeric(names(output$k_freqtab)[which.max(as.numeric(output$k_freqtab))])
    ...
    if(run_no %in% runs_to_plot){
        ...
        sigma2_vec = unlist(output$var_list_by_k_stephens[[1]])
        ...
        } else if(mod_type == "DEV"){
          
        } else{
          
          print("Traceplot of sigma2 not currently supported for this model type")
        }
...
data_path = "/projects/jklus/JKSTproj/BlueHive_Sim_Results/SummaryManuscript"
sim_dir = "MODSUM_conjDEE_3wellsep_n100_withSM_sim_results_2024_11_06"  
...
### DEV n=100 with split/merge

```{r}
data_path = "/projects/jklus/JKSTproj/BlueHive_Sim_Results/SummaryManuscript"
sim_dir = "MODSUM_conjDEV_3wellsep_n100_withSM_sim_results_2024_11_05"  
