...
set.seed(516)
n_iter = 10^4
ex1 = MVN_CRP_sampler(S = n_iter, y = y, alpha = 1, Sigma0 = diag(300, 2),
                        mu0 = matrix(data = rep(0, 2), nrow = 2), a = 1/2, b = 10,
...
group_assign_list_by_k = get_assign_by_k(assign = ex1$group_assign,
                                         n_groups = ex1$k,
                                         burn_in = 100)
make_postsum(mcmc_df = mean_list_by_k[[2]], digits = 2)
make_postsum(mcmc_df = mean_list_by_k[[3]], digits = 2)
make_traceplot(param_list_by_k = mean_list_by_k,
               k_index = 2,
               component_no = 1,
               param_type = "Mean",
               title_note = "K=3")
...
set.seed(516)
n_iter = 10^4
ex3 = MVN_CRP_sampler(S = n_iter, y = y, alpha = 1, Sigma0 = diag(300, 2),
                        mu0 = matrix(data = rep(0, 2), nrow = 2), a = 1/2, b = 10,
                        k_init = 1, verbose = TRUE, print_iter = 1000)

saveRDS(object = ex3, file = "ex3_results.rds")
ex3 = readRDS("ex3_results.rds")
...
group_assign_list_by_k3 = get_assign_by_k(assign = ex3$group_assign, 
                                         n_groups = ex3$k, 
                                         burn_in = 100)
make_postsum(mcmc_df = mean_list_by_k3[[2]], digits = 2)
make_postsum(mcmc_df = mean_list_by_k3[[3]], digits = 2)
make_postsum(mcmc_df = mean_list_by_k3[[4]], digits = 2)
make_traceplot(param_list_by_k = mean_list_by_k3, 
               k_index = 2, 
               component_no = 1,
               title_note = "K=2",
               param_type = "Mean")
...
set.seed(516)
n_iter = 10^3
ex4 = MVN_CRP_sampler(S = n_iter, y = y, alpha = 1, Sigma0 = diag(300, 2),
                        mu0 = matrix(data = rep(5.5, 2), nrow = 2), a = 1/2, b = 10,
                        k_init = 1, verbose = TRUE, print_iter = 1000)
...
var_list_by_k_stephens4 = list_params_by_k(draws = ex4$means, 
                                           burn_in = 100,
                                           relabel = TRUE,
                                           param_type = "Var",
                                           permutation = stephens_result4)
