...
```{r}
with_sm_res = readRDS(file = "../MCMC_Runs/conjUVVsamp_minisimstudy_close_withSM_4_Jan_2024.rds")
no_sm_res = readRDS(file = "../MCMC_Runs/conjUVVsamp_minisimstudy_close_noSM_4_Jan_2024.rds")
# with_sm_res[[1]]$settings
# no_sm_res[[1]]$settings

max(sapply(X = 1:10, FUN = function(x){max(as.numeric(names(table(with_sm_res[[x]]$k))))}))
max(sapply(X = 1:10, FUN = function(x){max(as.numeric(names(table(no_sm_res[[x]]$k))))}))
