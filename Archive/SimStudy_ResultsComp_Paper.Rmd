...
## What was the MAP estimate for K in each simulation?
...
summary_table %>% 
  ...
  tidyr::pivot_wider(names_from = c(Scenario, n_obs), values_from = MAP) %>%
  # keep first 2 columns with model info, sort remaining cols with results by n
  dplyr::select(1,2, gtools::mixedorder(names(.)[3:length(names(.))])+2) %>%
...
summary_table %>% 
  # dplyr::filter(Tag == "ENAR") %>%
  dplyr::group_by(Model, Scenario, SM, n_obs) %>%
  dplyr::summarize(MAP = round(mean(MAP_K),2)) %>%
  tidyr::pivot_wider(names_from = c(Scenario, n_obs), values_from = MAP) %>%
  # keep first 2 columns with model info, sort remaining cols with results by n
  dplyr::select(1,2, gtools::mixedorder(names(.)[3:length(names(.))])+2) %>%
  knitr::kable(caption = "Mean MAP value for K across 100 simulated data sets.")
