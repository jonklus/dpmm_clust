...
if(scenario == "3close"){
  ...
  p1 = ggplot(data = y30, aes(x = y1, y = y2)) + #, label = rownames(data))) +
    geom_point(color = assign, size = 0.75) +
    theme_classic() +
    theme(plot.title = element_text(size=14)) +
    ggtitle("n=30")
  
  p2 = ggplot(data = y100, aes(x = y1, y = y2)) + #, label = rownames(data))) +
    geom_point(color = assign, size = 0.75) +
    theme_classic() +
    theme(plot.title = element_text(size=14)) +
    ggtitle("n=100")
  
  p3 = ggplot(data = y300, aes(x = y1, y = y2)) + #, label = rownames(data))) +
    geom_point(color = assign, size = 0.75) +
    theme_classic() +
    theme(plot.title = element_text(size=14)) +
    ggtitle("n=300")
  
  png(filename = "./manuscript_plots/3close_3blockplot.png", res = 300, width = 6, height = 6, units = "in")
  ...
  png(filename = "./manuscript_plots/3wellsep_3blockplot.png", res = 300, width = 6, height = 6, units = "in")
  grid.arrange(p1, p2, p3, nrow = 1)
  dev.off()
