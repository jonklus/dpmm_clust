...
## Load a single file to investigate
...
result_wd = paste0(data_path, "/", sim_dir)
result_file = list.files(file_ext)
output = readRDS(paste0(result_wd, "/", result_file[5]))
output$settings
dim(output$group_assign)
iter_k = sapply(X = 1:nrow(output$group_assign), 
                FUN = function(x){
...
## Systematically pull info from all M=100 sim results
