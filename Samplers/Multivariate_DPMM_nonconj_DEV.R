...
group_prob_calc_DEV <- function(k, n, n_j, alpha, m = 1,
...
update_phi_DEV <- function(curr_label, group_assign, count_assign, y, 
  ...
  dens2 = LaplacesDemon::dinvgamma(x = Sigma[1], 
                                   shape = (p*count_assign + 2*a)/2, 
...
nonconj_phi_prob_DEV <- function(curr_label, group_assign, count_assign, y, 
                                  mu, mu0, Sigma, Sigma0, a, b,
                                 original_mu, original_sigma2){
  # function to calculate density full conditional posteriors of DEV model
  ...
  dens2 = LaplacesDemon::dinvgamma(x = original_sigma2, 
                                   shape = (p*count_assign + 2*a)/2, 
                                   scale = sum(loss_y_i)/2 + b, 
                                   log = TRUE)
...
nonconj_prior_dens_DEV <- function(mu, mu0, Sigma, Sigma0, a, b){
  ...
  dens = mvtnorm::dmvnorm(x = c(mu), mean = c(mu0), sigma = Sigma0, log = TRUE) + 
    LaplacesDemon::dinvgamma(x = sigma2, shape = a, scale = b, log = TRUE)
  ...
  for(s in 2:S){
      ...
      if(move_type == "SPLIT"){
        ...
        for(scan in 1:(sm_iter+1)){
          ...
          if(scan == sm_iter + 1){
            ...
            merge_phi_dens = nonconj_phi_prob_DEV(curr_label = merge_lab, 
                                 ...
                                 original_sigma2 = original_sigma1)
          } else{
            ...
            split_phi_prob = sapply(X = 1:2, 
                                    FUN = function(x){
                                      nonconj_phi_prob_DEV(
                                        ...
                                        original_sigma2 = c(original_sigma1, original_sigma2)[x])
        ...
        ### component 2 - denom I (group 1 - split proposal)
        prob3_denom1 = 0
        for(obs_ind in 1:length(subset_index_grp1)){
          val = ll_components_DEV(obs_ind = subset_index_grp1[obs_ind], y = y, 
                                  mu = original_mu1, 
        ...
        ### component 3 - denom II (group 2 - split proposal)
        prob3_denom2 = 0
        for(obs_ind in 1:length(subset_index_grp2)){
          val = ll_components_DEV(obs_ind = subset_index_grp2[obs_ind], y = y, 
    ...
    pr_c = sapply(X = 1:length(y), FUN = function(x){
      group_prob_calc_DEV(k = k, n = n, n_j = count_assign, alpha = alpha, a = a, b = b, 
                          ...
                          curr_labels = curr_labels, final_prob_calc = TRUE)$pr_c
