...
group_prob_calc_DVV <- function(k, n, n_j, alpha, y_i, mu, Sigma, mu0, Sigma0, a, b,
  ...
  k_minus = k-1 # number of distinct groups excluding the jth group
    ...
    Sigma_new = diag(x = 1/rgamma(n = 1, shape = a, rate = b), nrow = nrow(Sigma0))
...
MVN_CRP_nonconj_DVV <- function(S = 10^3, seed = 516, y, alpha = 1, 
                                k_init = 2, init_method = "kmeans",
                                mu0, Sigma0, a, b, a0, b0,
                                standardize_y = FALSE,
  ...
  # a, b are hyperpiors on IG variance prior
  # a0, b0 are hyperpriors on diagonal elements of prior variance Sigma0
  ...
  colnames(extra_params) = c("sigma0")
  extra_params[1,1] = Sigma0[1]
  ...
  for(s in 2:S){
    ...
    for(i in 1:n){
      ...
      if(group_assign[s,i] %in% singletons){
        ...
        pr_res = group_prob_calc_DVV(k = k, n = n, n_j = count_assign, alpha = alpha, 
                               y_i = y[[i]], mu = mu, Sigma = Sigma,
                               mu0 = mu0, Sigma0 = Sigma0, a = a, b = b,
                               singleton = 1)
        pr_c = pr_res$pr_c
    ...
    # due to DVV design, need to pick out the jth diagonal element of the Kth 
    # covariance matrix, which corresponds to the jth entry in the mean vector
    # of the Kth mixture component
    ...
    Sigma = lapply(X = 1:k, 
                    FUN = function(x){
                      ...
                      # vectorized rgamma to be p-dim and use array of loss
                      # function reults from loss_y_i
                      diag(1/rgamma(n = p, 
                              shape = rep(count_assign[x]/2 + a, p),
                              rate = loss_y_i[[x]]/2 + rep(b, p)))
                    })
    

    # draw Sigma0 = sigma0*I
    loss_mu =  unlist(lapply(X = 1:k, 
                  FUN = function(x){
                          t(mu[,x] - mu0)%*%(mu[,x] - mu0)
                  }))
    
    Sigma0 = diag(rep(1/rgamma(n = 1, shape = k*p/2 + a0, 
                           rate = sum(loss_mu)/2 + b0), p),
                  ncol = p)
    
    # save draws of mu and Sigma
    means[[s]] = mu
    vars[[s]] = Sigma
    extra_params[s,"sigma0"] = Sigma0[1]
    ...
    pr_c = sapply(X = 1:length(y), FUN = function(x){
      group_prob_calc_DVV(k = k, n = n, n_j = count_assign, alpha = alpha, 
