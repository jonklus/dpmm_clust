...
# sbatch-fed settings
n_array = c(30,100,300)[2]
# i = as.numeric(Sys.getenv("i"))
# n = n_array[i]
cat("\n n=", n, "\n")

dir_name = paste0("./SummaryMultichain/MODSUM_", model, "_", scenario, "_n", n, 
...
# simulate data
# for multichain use same seed for all data generation
set.seed(516)
...
if(scenario == "3close"){
  ...
  g = 1; h = 25
  sigma0 = 150
  lambda0 = diag(x = 20, nrow = length(means[[1]])) # needed for UVV
  nu = length(means[[1]]) + 2  # must have nu >= p...Otherwise rinvwish will fail!
  # set nu = p + 2 so that E[lambda0]>0, otherwise may go negative which would be bad
  ...
  g = 1; h = 50
  sigma0 = 225
  lambda0 = diag(x = 50, nrow = length(means[[1]])) # needed for UVV
  nu = length(means[[1]]) + 2  # must have nu >= p...Otherwise rinvwish will fail!
  # set nu = p + 2 so that E[lambda0]>0, otherwise may go negative which would be bad
...
if(model == "conjDEE"){
  ...
  try(expr = {
    ...
    output = MVN_CRP_sampler_DEE(
      ...
      alpha = 1, a = a, b = b, r = 1, d = d, f = f, g = g, h = h,
      ...
      k_init = SLURM_ID, init_method = "kmeans", diag_weights = FALSE,
      verbose = FALSE, split_merge = SM, sm_iter = sm_iter)
  ...
  try(expr = {
    ...
    output = MVN_CRP_sampler_UVV(
      S = 12000, seed = seeds[SLURM_ID], y = y,
      alpha = 1, r = 1, g = g, h = h, nu = nu, fix_r = FALSE,
      mu0 = matrix(round((colMeans(matrix(unlist(y), ncol = p))),0), ncol = 1),
      lambda0 = lambda0,
      truth = list(mu_true = means, var_true = var, assign_true = assign),
      k_init = SLURM_ID, init_method = "kmeans", diag_weights = FALSE,
